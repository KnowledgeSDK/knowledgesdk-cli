...
import ora from 'ora';
import { extractRun, extractStream } from '../lib/api.js';
import {
  colorizeStatus,
  handleApiError,
  printField,
...
export function registerExtractCommand(program: Command): void {
  program
    .command('extract <url>')
    .description('Run full AI knowledge extraction on a website (classify, crawl, and extract)')
    ...
    .action(
      async (
        ...
        if (opts.async) {
          ...
          try {
            const result = await extractRun(url, {
              async: true,
        ...
        if (opts.stream !== false) {
          ...
          try {
            for await (const event of extractStream(url, { maxPages: opts.maxPages })) {
              switch (event.type) {
                case 'connected':
        ...
        const spinner = ora({ text: `Running extraction on ${theme.url(url)}…`, color: 'cyan' }).start();
        try {
          const result = await extractRun(url, {
            async: false,
            callbackUrl: opts.callbackUrl,
