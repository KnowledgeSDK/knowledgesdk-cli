...
import ora from 'ora';
import { businessExtract, businessExtractStream } from '../lib/api.js';
import {
  colorizeStatus,
  handleApiError,
  printField,
...
import type { BusinessOptions } from '../types.js';

export function registerBusinessCommand(program: Command): void {
  program
    .command('business <url>')
    .description('Run full AI extraction on a business website (classify, crawl, and extract)')
    ...
    .action(
      async (
        url: string,
        opts: BusinessOptions & { json?: boolean; stream?: boolean },
      ) => {
        // ── Async mode: fire and forget ──────────────────────────────────────
        if (opts.async) {
          const spinner = ora({ text: `Queuing business extraction for ${theme.url(url)}…`, color: 'cyan' }).start();
          try {
            const result = await businessExtract(url, {
              async: true,
            ...
            printHeader('Business extraction queued');
        ...
        if (opts.stream !== false) {
          ...
          try {
            for await (const event of businessExtractStream(url, { maxPages: opts.maxPages })) {
              switch (event.type) {
                case 'connected':
            ...
            if (finalResult) {
              console.log('');
              printHeader('Business extraction complete');
        ...
        const spinner = ora({ text: `Running business extraction on ${theme.url(url)}…`, color: 'cyan' }).start();
        try {
          const result = await businessExtract(url, {
            async: false,
            callbackUrl: opts.callbackUrl,
          ...
          spinner.stop();
          if (opts.json) { printJson(result); return; }
          printHeader('Business extraction result');
          printField('Job ID', result.jobId);
          printField('Status', colorizeStatus(result.status));
          printField('URL', result.url);
